    ...
    <div className="rounded-xl border border-slate-900 bg-slate-955/20 p-4 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex-1">
          {isLoading ? (
            ...
            <select
              ...
              className="w-full rounded-lg border border-slate-800 bg-slate-955 px-3 py-2 text-sm text-slate-200 focus:border-emerald-600 focus:outline-none focus:ring-1 focus:ring-emerald-600 transition-all"
            >
              ...
              {availableProducts.map((prod) => (
                <option key={prod.id} value={prod.id} className="bg-slate-955">
                  {prod.name} (SKU: {prod.sku}) - ₹{parseFloat(prod.unit_cost).toFixed(2)}
                </option>
              ))}
            </select>
          )}
        ...
        <button
          ...
          className="flex items-center justify-center space-x-1.5 rounded-lg bg-emerald-600 px-4 py-2 text-xs font-bold text-white hover:bg-emerald-500 transition-all disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
        >
