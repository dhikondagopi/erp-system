...
import { ArrowLeft, CheckCircle, Send, XCircle, AlertTriangle, FileText } from 'lucide-react';
import { useAuth } from '../../../context/AuthContext';
import { usePurchaseOrderDetailsQuery, useUpdateOrderStatusMutation } from '../hooks/usePurchase';
...
const PurchaseOrderDetailsPage = () => {
  ...
  const { data: order, isLoading, error } = usePurchaseOrderDetailsQuery(id);
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'PENDING_APPROVAL':
        return (
          <span className="inline-block rounded-full bg-amber-955/20 border border-amber-900/30 px-3 py-1 text-xs font-bold text-amber-400 uppercase tracking-wide">
            Pending Approval
          </span>
        );
      case 'APPROVED':
  ...
  if (isLoading) {
    return (
      <div className="flex h-[50vh] flex-col items-center justify-center space-y-3">
        ...
        <p className="text-xs text-slate-500 font-semibold">Loading purchase order details...</p>
      </div>
    );
  }
  ...
  if (error || !order) {
    return (
      <div className="rounded-xl border border-red-955/40 bg-red-955/20 p-6 text-center max-w-lg mx-auto">
        ...
        <h3 className="text-sm font-bold text-red-400">Failed to load purchase order</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-900 pb-5">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Procurement Ledger</span>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {hasRole(['Admin', 'Purchase User']) && order.status === 'DRAFT' && (
            <>
              <button
          ...
          {hasRole(['Admin', 'Business Owner']) && order.status === 'PENDING_APPROVAL' && (
            <>
              <button
              ...
              <button
                ...
                disabled={statusMutation.isLoading}
                className="flex items-center space-x-2 rounded-lg bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs px-4 py-2.5 shadow-md shadow-rose-900/20 transition-all disabled:opacity-50"
              >
                <XCircle className="h-4 w-4" />
          ...
          {order.status === 'APPROVED' && (
            <>
              ...
              {hasRole(['Admin', 'Purchase User', 'Business Owner']) && (
                <button
                  onClick={() => handleStatusTransition('CANCELLED')}
                  disabled={statusMutation.isLoading}
                  className="flex items-center space-x-2 rounded-lg border border-slate-800 bg-slate-950 hover:bg-red-955/20 hover:text-red-400 hover:border-red-900/35 text-slate-455 font-bold text-xs px-4 py-2.5 transition-all disabled:opacity-50"
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-xl border border-slate-900 bg-slate-955/40 p-5 space-y-3">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Supplier Profile</span>
              <div className="space-y-1">
                <p className="text-sm font-bold text-white">{order.vendor_name}</p>
                <p className="text-xs text-slate-400">{order.vendor_email}</p>
              </div>
            </div>
                  ...
                  {order.items?.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-900/20">
                      ...
                      <td className="px-6 py-3.5 font-mono font-semibold text-slate-200">
                        ₹{(parseFloat(item.quantity) * parseFloat(item.unit_cost)).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-slate-900 bg-slate-955/40 p-5 space-y-4">
            ...
            <div className="relative border-l border-slate-800 pl-4 ml-2.5 space-y-5">
              ...
              {(order.status === 'PENDING_APPROVAL' || order.status === 'APPROVED' || order.status === 'RECEIVED' || order.status === 'REJECTED') && (
                <div className="relative animate-in fade-in duration-200">
                  <div className="absolute -left-6.5 top-0.5 rounded-full bg-slate-950 p-1 border border-amber-900/40 text-amber-400">
                    <Send className="h-3 w-3" />
                  </div>
                  <div>
                    <h4 className="text-xs font-bold text-white">Submitted for Approval</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5">Awaiting Owner decision</p>
                  </div>
                </div>
              )}
              ...
              {(order.status === 'APPROVED' || order.status === 'RECEIVED') && (
                <div className="relative animate-in fade-in duration-200">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 mt-0.5">Approved by Owner, incoming stock mapped</p>
                  </div>
                </div>
              )}
              ...
              {order.status === 'REJECTED' && (
                <div className="relative animate-in fade-in duration-200">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-white">Order Rejected</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5">Rejected by Owner</p>
                  </div>
                </div>
              )}
              ...
              {order.status === 'RECEIVED' && (
                <div className="relative animate-in fade-in duration-200">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 mt-0.5">Physical stocks increased & average cost updated</p>
                  </div>
                </div>
              )}
              ...
              {order.status === 'CANCELLED' && (
                <div className="relative animate-in fade-in duration-200">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 mt-0.5">Expectations released or order voided</p>
                  </div>
                </div>
              )}
            </div>
