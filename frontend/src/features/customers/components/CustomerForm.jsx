...
const CustomerForm = ({ initialValues, onSubmit, isSubmitting, isEdit = false }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl mx-auto rounded-xl border border-slate-900 bg-slate-950/40 p-6 backdrop-blur-md">
      {/* Customer Name */}
      <div>
        <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Customer / Company Name</label>
        <input
          type="text"
          name="name"
          value={form.name}
          onChange={handleChange}
          placeholder="e.g. Shiv Furniture Works, John Doe"
      ...
      {/* Address */}
      <div>
        <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Billing / Shipping Address</label>
        <textarea
          name="address"
          rows="4"
          value={form.address}
          onChange={handleChange}
          placeholder="Enter corporate office street details, city, postal code..."
          className="w-full rounded-lg border border-slate-800 bg-slate-950/20 px-3 py-2 text-sm text-slate-200 focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600 transition-all"
        />
      </div>
      ...
      <div className="flex justify-end space-x-3.5 pt-4 border-t border-slate-900">
        ...
        <button
          ...
          className="px-5 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-500 font-semibold text-sm shadow-md shadow-blue-900/20 transition-all active:scale-95 disabled:opacity-50"
        >
          {isSubmitting ? 'Saving changes...' : isEdit ? 'Update Profile' : 'Register Customer'}
        </button>
      </div>
    </form>
  );
